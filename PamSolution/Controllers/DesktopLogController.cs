            ...
            try
            {
                using (var ctx = new PamProjectEntities2())
                {
                    ...
                    if (userSession.expireTime >= DateTime.Now)
                    {
                        ...
                        string sql = "INSERT INTO DesktopLog (userId, accessTime, logContentLocation, permissionLevelId, finishTime, userNote, protectedAccountId) VALUES (" + userInfo.userId + ",'" + DateTime.Now.AddHours(12).ToString("yyyy-MM-dd HH:mm:ss.fff") + "','" + postUser.LogContentLocation + "'," + userInfo.permissionLevelId + ",'" + DateTime.Now.AddHours(12).ToString("yyyy-MM-dd HH:mm:ss.fff") + "','" + postUser.UserNote + "'," + postUser.ProtectedAccountId + ");";
